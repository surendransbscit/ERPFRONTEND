...
const OldTagCodeAutoComplete = ({ id, placeholder, SetValue, SetSearchValue, searchValue,selectedBranch }) => {
  ...
  useEffect(() => {
    ...
    if (isSearching && searchValue.length > 0 && searchValue[0].label.length >= 4 && selectedBranch) {
        dispatch(getTagCodeBySearch({ id_branch:selectedBranch,old_tag_code : searchValue[0]?.label }));
...
export default OldTagCodeAutoComplete;
