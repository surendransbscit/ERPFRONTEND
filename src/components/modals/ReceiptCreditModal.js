import React from "react";
import { Button, Modal, ModalBody, ModalHeader } from "reactstrap";
import { Col, Icon, Row } from "../Component";
import { toastfunc } from "../sds-toast-style/toast-style";

const ReceiptCreditModal = ({ modal, toggle, data, SetData, creditDataAmount, discountAmount }) => {
  const updateAmount = async (obj, value) => {
    SetData((prevState) =>
      prevState?.map((item) => {
        if (item == obj) {
          if (parseFloat(value) + parseFloat(obj.discount) > parseFloat(obj.balance_amount)) {
            toastfunc("You can't add more than the balance");
          } else {
            return {
              ...item,
              amount: value,
            };
          }
        }
        return item;
      })
    );
  };
  const updateDiscount = async (obj, value) => {
    SetData((prevState) =>
      prevState?.map((item) => {
        if (item == obj) {
          if (parseFloat(obj.amount) + parseFloat(value) > parseFloat(obj.balance_amount)) {
            toastfunc("You can't add more than the balance");
          } else {
            return {
              ...item,
              discount: value,
            };
          }
        }
        return item;
      })
    );
  };
  return (
    <Modal isOpen={modal} className="modal-dialog-centered text-center" size="lg">
      <ModalHeader
        tag="h6"
        className="bg-light"
        toggle={toggle}
        close={
          <button
            className="close"
            style={{
              position: "absolute",
              right: "1rem",
            }}
            onClick={toggle}
          >
            <Icon name="cross" />
          </button>
        }
      >
        <span style={{ fontSize: "small" }}>Credit Collection List</span>
      </ModalHeader>
      <ModalBody className="text-center ">
        <div className="table-responsive mt-4">
          <table className="table table-bordered">
            <thead>
              <tr>
                <th>S.NO</th>
                <th>Issue No</th>
                <th>Amount</th>
                <th>Paid</th>
                <th>Balance</th>
                <th>Discount</th>
                <th>Received Amount </th>
              </tr>
            </thead>
            <tbody>
              {data?.map((item, idx) => {
                return (
                  <tr key={idx}>
                    <td>{idx + 1}</td>
                    <td>{item.bill_no}</td>
                    <td>{item.issued_amount}</td>
                    <td>{item.received_amount}</td>
                    <td>{item.balance_amount}</td>

                    <td>
                      <div className="form-control-wrap">
                        <input
                          type="number"
                          min={0}
                          className="form-control form-control-sm no-spinner"
                                    onWheel={(e) => e.target.blur()}
                          id={`${item?.id}-discount`}
                          value={item?.discount}
                          onChange={(e) => updateDiscount(item, e.target.value)}
                        />
                        <label className="form-label" htmlFor={`${item?.id}-discount`}></label>
                      </div>
                    </td>
                    <td>
                      <div className="form-control-wrap">
                        <input
                          type="number"
                          min={0}
                          className="form-control form-control-sm no-spinner"
                                    onWheel={(e) => e.target.blur()}
                          id={`${item?.id}-amount`}
                          value={item?.amount}
                          onChange={(e) => updateAmount(item, e.target.value)}
                        />
                        <label className="form-label" htmlFor={`${item?.id}-amount`}></label>
                      </div>
                    </td>
                  </tr>
                );
              })}
            </tbody>
            {/* <tfoot>
              {paymentHistoryList.payments?.map((item, idx) => {
                return (
                  <tr key={idx} style={{ fontWeight: "bold" }}>
                    <td>Total</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td>{item.payment_amount}</td>
                    <td>{item.net_amount}</td>
                    <td>{item.metal_weight}</td>
                    <td></td>
                  </tr>
                );
              })}
            </tfoot> */}
          </table>
        </div>

        <Row>
          <Col md={6}></Col>
          <Col md={3}>
            <div className="form-control-wrap">
              <input
                readOnly
                type="number"
                min={0}
                className="form-control form-control-sm no-spinner"
                                    onWheel={(e) => e.target.blur()}
                id={`total_received_amount`}
                value={discountAmount}
              />
              <label className="form-label" htmlFor={`total_received_amount`}>
                Total Discount
              </label>
            </div>
          </Col>
          <Col md={3}>
            <div className="form-control-wrap">
              <input
                readOnly
                type="number"
                min={0}
                className="form-control form-control-sm no-spinner"
                                    onWheel={(e) => e.target.blur()}
                id={`total_received_amount`}
                value={creditDataAmount}
              />
              <label className="form-label" htmlFor={`total_received_amount`}>
                Total Received
              </label>
            </div>
          </Col>
        </Row>
        <Row>
          <Col md={6}></Col>
          <Col md={3}></Col>
          <Col md={3}>
            <Button onClick={() => toggle({ Save: true })} color="primary" className="mt-3 mb-2" size="md">
              Save
            </Button>
          </Col>
        </Row>
      </ModalBody>
    </Modal>
  );
};

export default ReceiptCreditModal;
