...
const OtherChargesForm = () => {
  ...
  const { otherChargesInfo } = useSelector((state) => state.otherChargesReducer);
  ...
  const postData = async () => {
    ...
    try {
      await dispatch(createOtherCharges(adddata)).unwrap();
      toastsuccess(name + " Added successfully");
      navigate(`${process.env.PUBLIC_URL}/master/othercharges/list`);
    } catch (error) {
      console.error(error);
    }
  ...
  const postAndCreateNew = async () => {
    ...
    await dispatch(createOtherCharges(adddata));
    if (isError === false) {
      toastsuccess("Other Charges Added successfully");
      setName("");
      setAmount("");
      setActive(true);
    }
  ...
  useEffect(() => {
    id !== undefined && dispatch(getOtherChargesById(id));
  }, [dispatch, id]);

  useEffect(() => {
    otherChargesInfo != undefined &&
      (setName(otherChargesInfo?.name),
        setAmount(otherChargesInfo?.amount),
        setActive(otherChargesInfo?.is_active),
        reset());
  }, [otherChargesInfo, reset]);
  ...
  const putData = async () => {
    ...
    await dispatch(updateOtherChargesById(reduxData));
    if (isError === false) {
      toastsuccess("Other Charges Edited successfully");
      navigate(`${process.env.PUBLIC_URL}/master/othercharges/list`);
    }
  };
  ...
  return (
    <React.Fragment>
      <Head title={title ? title : "Other Charges"} />
      <Content>
