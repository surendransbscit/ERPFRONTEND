...
import { getOrders, cancelOrders, getAssignedOrders } from "../../../redux/thunks/Order";
...
const CustomerOrderStatus = () => {
  ...
  const form_submit = async (data, actionType) => {
    ...
    orderList.map((item, rowIndex) => {
      if (item.isChecked)
        assignData.push({ detail_id: item.detail_id, cancel_reason: item.cancel_reason ? item.cancel_reason : "" });
