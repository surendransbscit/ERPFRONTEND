    ...
    <>
      <Row className="form-group row g-4 ">
        {data?.map((item) =>
          item?.show ? (
            <>
              {/* <Col md="2" key={`label-${item.id}`} className="text-end"> */}
              <Col md="2" key={`label-${item.id}`}>
                <label className="form-label" htmlFor={`input-${item.id}`}>
                  {item?.description}
                  {(item.description === "Ask($)" ||
                    item.description === "INR") && (
              ...
              <Col md="2" key={`value-${item.id}`}>
                {item.type === "input" || item.type === "rate" ? (
                  <input
                    id={`input-${item.id}`}
                    type="number"
                    className="form-control no-spinner"
                                    onWheel={(e) => e.target.blur()}
                    value={bankMetal === 1 ? item.goldValue : item.silverValue}
                    onChange={(e) =>
                      handleInputChange(
                        item.id,
                        bankMetal === 1 ? "goldValue" : "silverValue",
                        e.target.value
                      )
                    }
                  />
...
export default BuyAtBankComponent;
