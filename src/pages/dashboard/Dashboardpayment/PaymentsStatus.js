...
import { getPaymentStatus } from "../../../redux/thunks/dashboard";

const PaymentsStatus = () => {
  const [days, SetDays] = useState();
  const dispatch = useDispatch();
  const { paymentStatusList } = useSelector((state) => state.dashboardReducer);
  ...
  const statusData = paymentStatusList?.map((obj, idx) => {
    const container = {};
  ...
  useEffect(() => {
    dispatch(getPaymentStatus({ days: days }));
  }, [dispatch, days]);
          ...
          <div className="card-inner flex-grow-1">
            <div className="card-title-group mb-4">
              <div className="card-title">
                <h6 className="title">Payment Status</h6>
              </div>
            ...
            {paymentStatusList?.length > 0 ? (
              <div className="data-group">
                <div className="nk-ecwg4-ck">
                  <TrafficSourcesChart state={statusData} />
                </div>
