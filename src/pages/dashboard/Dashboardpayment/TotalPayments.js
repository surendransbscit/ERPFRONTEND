...
import { getPaymentModeDash } from "../../../redux/thunks/dashboard";
import { useDispatch, useSelector } from "react-redux";

const TotalPayments = () => {
  const [days, SetDays] = useState();
  const dispatch = useDispatch();
  const { paymentModeDashList } = useSelector((state) => state.dashboardReducer);
  ...
  const modeData = paymentModeDashList?.map((obj, idx) => {
    const container = {};
    container.mode_name = obj.mode_name;
  ...
  useEffect(() => {
    dispatch(getPaymentModeDash({ days: days }));
  }, [dispatch, days]);
          ...
          <div className="card-inner flex-grow-1">
            <div className="card-title-group mb-4">
              <div className="card-title">
                <h6 className="title">Payment Mode</h6>
              </div>
            ...
            {modeData?.length > 0 ? (
              <div className="data-group">
                <div className="nk-ecwg4-ck">
                  <TrafficSourcesChart state={modeData} />
                </div>
