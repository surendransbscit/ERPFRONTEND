...
import { getBranchWiseSchemeClosed } from "../../../redux/thunks/dashboard";

const BranchWiseSchemeClosed = () => {
  const [days, SetDays] = useState();
  const dispatch = useDispatch();
  const { branchWiseSchemeClosedList } = useSelector((state) => state.dashboardReducer);
  ...
  const closedData = branchWiseSchemeClosedList?.map((obj, idx) => {
    const container = {};
  ...
  useEffect(() => {
    dispatch(getBranchWiseSchemeClosed({ days: days }));
  }, [dispatch, days]);
          ...
          <div className="card-inner flex-grow-1">
            <div className="card-title-group mb-4">
              <div className="card-title">
                <h6 className="title">Branch Wise Scheme Closed</h6>
              </div>
...
export default BranchWiseSchemeClosed;
