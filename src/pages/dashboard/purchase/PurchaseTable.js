...
import { getPurchaseDetailList } from "../../../redux/thunks/purchaseDashboard";

const PurchaseTable = () => {
    const navigate = useNavigate()
    const dispatch = useDispatch();
    const { purchaseDetailList } = useSelector((state) => state.retailDashboardReducer);

    const [days, SetDays] = useState("4");
    useEffect(() => {
        dispatch(getPurchaseDetailList({ view: days }));
    }, [dispatch, days]);
    return (
            ...
            <Table>
                <thead>
                    <tr>
                        ...
                        <th className="text-right">Pure Weight</th>
                    </tr>
                </thead>
                <tbody>
                    {purchaseDetailList?.length > 0 ? (
                        <>
                            {purchaseDetailList?.map((item, idx) => (
                                <tr key={idx} style={{
                                    cursor:"pointer"
                                  }}
                                //   onClick={() => {
