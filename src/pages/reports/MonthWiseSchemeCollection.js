...
import { getMonthWiseSchemeCollection } from "../../redux/thunks/reports";
...
const MonthWiseSchemeCollection = () => {
  const { register, clearErrors, setValue, errors } = useForm();
  const location = useLocation();
  const dispatch = useDispatch();
  const pathName = location?.pathname;

  const { monthWiseSchemeCollectionList, isLoading: loadingData } = useSelector((state) => state.reportReducer);
  ...
  const columns = [
    ...
    { accessor: "jan_total", header: "Jan", isCurrency: true },
    { accessor: "feb_total", header: "Feb", isCurrency: true },
    { accessor: "mar_total", header: "Mar", isCurrency: true },
    { accessor: "apr_total", header: "Apr", isCurrency: true },
    { accessor: "may_total", header: "May", isCurrency: true },
    { accessor: "jun_total", header: "Jun", isCurrency: true },
    { accessor: "jul_total", header: "Jul", isCurrency: true },
    { accessor: "aug_total", header: "Aug", isCurrency: true },
    { accessor: "sep_total", header: "Sep", isCurrency: true },
    { accessor: "oct_total", header: "Oct", isCurrency: true },
    { accessor: "nov_total", header: "Nov", isCurrency: true },
    { accessor: "dec_total", header: "Dec", isCurrency: true },
  ];
  ...
  const FilterComponent = (
    <ReportFilterComponent
      children={{
        ...
        is_added_through_filter_req: false,
      }}
    />
  );
  ...
  return (
    <React.Fragment>
      <Head title={pagePermission?.title ? pagePermission?.title : "Month Wise Scheme Collection Reports"}></Head>
      {pagePermission?.view && (
        <Content>
          <Block size="lg">
              ...
              <Styles>
                <div className="card-inner">
                  <div className="card-title-group">
                    <div className="toggle-wrap nk-block-tools-toggle">
                      <h5>Month Wise Scheme Collection Reports</h5>
                    </div>
                ...
                <div className="table-responsive dataTables_wrapper">
                  <table className="table-wrapper react_table">
                    ...
                    <tbody>
                      {monthWiseSchemeCollectionList?.length > 0 &&
                        monthWiseSchemeCollectionList?.map((item, rowIndex) => (
                          <>
...
export default MonthWiseSchemeCollection;
