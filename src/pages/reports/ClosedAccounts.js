...
import { getClosedAccountReport } from "../../redux/thunks/reports";
...
const ClosedAccounts = () => {
  ...
  const { closedAccountReportList, isLoading: loadingData } = useSelector((state) => state.reportReducer);
  ...
  const columns = closedAccountReportList?.columns?.map((col, index) => {
    if (col.selector === "sno") {
  ...
  return (
    <React.Fragment>
      <Head title={pagePermission?.title ? pagePermission?.title : "Closed Account Report"}></Head>
      {pagePermission?.view && (
        <Content>
          <Block size="lg">
            <ReactDataTable
              title={pagePermission?.title ? pagePermission?.title : "Closed Account Report"}
              columns={closedAccountReportList?.columns ? columns : []}
              data={closedAccountReportList?.rows ? closedAccountReportList?.rows : []}
