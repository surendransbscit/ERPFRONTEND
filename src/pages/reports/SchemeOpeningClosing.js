...
import { getSchemeWiseOpeningReport } from "../../redux/thunks/reports";
import moment from "moment";
import { useForm } from "react-hook-form";
import { getAllScheme } from "../../redux/thunks/scheme";

const SchemeOpeningClosing = () => {
  ...
  const { schemeWiseOpeningReportList, isLoading: loadingData } = useSelector((state) => state.reportReducer);
  ...
  const columns = schemeWiseOpeningReportList?.columns?.map((col, index) => {
    if (col.selector === "sno") {
  ...
  return (
    <React.Fragment>
      <Head title={pagePermission?.title ? pagePermission?.title : "Scheme Wise Opening Report"}></Head>
      {pagePermission?.view && (
        <Content>
          <Block size="lg">
            <ReactDataTable
              title={pagePermission?.title ? pagePermission?.title : "Scheme Wise Opening Report"}
              columns={schemeWiseOpeningReportList?.columns ? columns : []}
              data={schemeWiseOpeningReportList?.rows ? schemeWiseOpeningReportList?.rows : []}
...
export default SchemeOpeningClosing;
