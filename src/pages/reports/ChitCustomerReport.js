...
import { getChitCustomerReport, getMonthWiseSchemeJoin } from "../../redux/thunks/reports";
...
const ChitCustomerReport = () => {
  const { register, clearErrors, setValue, errors } = useForm();
  const location = useLocation();
  const dispatch = useDispatch();
  const pathName = location?.pathname;

  const { chitCustomerList, isLoading: loadingData } = useSelector((state) => state.reportReducer);
  ...
  return (
    <React.Fragment>
      <Head title={pagePermission?.title ? pagePermission?.title : "Chit Customer Reports"}></Head>
      {pagePermission?.view && (
        <Content>
          <Block size="lg">
              ...
              <Styles>
                ...
                <div className="table-responsive dataTables_wrapper">
                  <table className="table-wrapper react_table">
                    ...
                    <tbody>
                      {chitCustomerList?.length > 0 &&
                        chitCustomerList?.map((item, rowIndex) => (
                          <>
...
export default ChitCustomerReport;
