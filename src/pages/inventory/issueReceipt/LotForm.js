...
import { createLotIssueReceiptForm, getLotStockList } from "../../../redux/thunks/inventory";
import { employee_id } from "../../../redux/configs";


const LotDetailsIssueReceiptForm = () => {
  ...
  const getStockDetails = () => {
    ...
    if (branch) {
      ...
      if(Type == 2){
        ...
        filter = `lot_id=${lotCode}`
      }
      dispatch(getLotStockList(filter));
    } else if (!branch) {
      ...
      stockList?.map((item, rowIndex) => {
        if (item.isChecked) {
          ...
          assignData.push({
            ...
            "receipt_pieces":item.pieces,
            "receipt_gross_wt":item.gross_wt,
            "receipt_less_wt":item.less_wt,
            "receipt_net_wt":item.net_wt,
          })
        }
      })
      if(!employee){
  ...
  const createStockApproval = async (data) => {
    try {
      let response = await dispatch(createLotIssueReceiptForm(data)).unwrap();
...
export default LotDetailsIssueReceiptForm;
